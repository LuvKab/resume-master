...
export function SidePanel() {
  ...
  const sectionHeights = globalSettings?.sectionHeights || {};
  ...
  const getDefaultSectionHeight = (sectionId: string) => {
    if (sectionId === "basic") return 190;
    if (sectionId === "skills") return 150;
    return 170;
  };

  const updateSectionHeight = (sectionId: string, value: number) => {
    const nextValue = Math.max(60, Math.min(420, Math.round(value)));
    updateGlobalSettings?.({
      sectionHeights: {
        ...(globalSettings?.sectionHeights || {}),
        [sectionId]: nextValue,
      },
    });
  };

  const resetSectionHeight = (sectionId: string) => {
    const nextHeights = { ...(globalSettings?.sectionHeights || {}) };
    delete nextHeights[sectionId];
    updateGlobalSettings?.({ sectionHeights: nextHeights });
  };
  return (
    <motion.div
      className={cn(
        ...
        <SettingCard icon={Layout} title={t("layout.title")}>
          ...
          <div className="space-y-4 pt-3 border-t border-border/60">
            <Label className="text-muted-foreground">
              {t("layout.sectionHeight.title")}
            </Label>
            <div className="space-y-3">
              {orderedSections.map((section) => {
                const currentHeight = sectionHeights?.[section.id];
                const sliderValue =
                  typeof currentHeight === "number" && currentHeight > 0
                    ? currentHeight
                    : getDefaultSectionHeight(section.id);
                ...
                return (
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => resetSectionHeight(section.id)}
                        className="text-xs text-muted-foreground hover:text-foreground transition-colors"
                      >
                        {t("layout.sectionHeight.auto")}
                      </button>
                    </div>
                    <div className="flex items-center gap-3">
                      <Slider
                        ...
                        step={2}
                        onValueChange={([value]) =>
                          updateSectionHeight(section.id, value)
                        }
                        className="flex-1"
                      />
                      <div className="flex items-center text-xs text-muted-foreground min-w-[70px] justify-end">
                        {typeof currentHeight === "number" && currentHeight > 0
                          ? `${currentHeight}px`
                          : t("layout.sectionHeight.autoValue")}
                      </div>
                    </div>
                  </div>
                );
