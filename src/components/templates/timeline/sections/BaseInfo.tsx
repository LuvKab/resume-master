...
import { isExternalHref, resolveBasicFieldHref } from "@/lib/basicFieldLink";
        ...
        <SectionWrapper sectionId="basic">
            <div className={styles.container}>
                <div className={styles.leftContent}>
                    ...
                    <div className={cn("flex flex-col", styles.nameTitle)}>
                        {nameField.visible !== false && basic[nameField.key] && (
                            <motion.h1 layout="position" className="font-bold whitespace-pre-wrap break-words" style={{ fontSize: `${nameFontSize}px` }}>{basic[nameField.key] as string}</motion.h1>
                        )}
                        {titleField.visible !== false && basic[titleField.key] && (
                            <motion.h2 layout="position" className="whitespace-pre-wrap break-words" style={{ fontSize: `${titleFontSize}px` }}>{basic[titleField.key] as string}</motion.h2>
                        )}
                    </div>
                </div>
                <motion.div layout="position" className={styles.fields} style={{ fontSize: `${globalSettings?.baseFontSize || 14}px`, color: "rgb(75, 85, 99)", maxWidth: layout === "center" ? "none" : "600px" }}>
                    {allFields.map((item) => {
                        const href = resolveBasicFieldHref(String(item.key), String(item.value), Boolean(item.custom));
                            ...
                            <motion.div key={item.key} className="flex items-center whitespace-nowrap overflow-hidden text-baseFont">
                                {useIconMode ? (
                                    <div className="flex items-center gap-1">
                                        {getIcon(item.icon)}
                                        {href ? (
                                            <a href={href} className="underline" {...linkProps}>
                                                {item.value}
                                            </a>
                                        ) : (
                                    ...
                                    <div className="flex items-center gap-2 overflow-hidden">
                                        ...
                                        {href ? (
                                            <a href={href} className="truncate underline" suppressHydrationWarning {...linkProps}>
                                                {item.value}
                                            </a>
                                        ) : (
